...
async function startServer() {
  ...
  app.post(['/api/ai-chat', '/api/ai/chat'], async (req, res) => {
    try {
      ...
      const { audioData, audioBase64, mimeType, messages, history, mode, lang, context } = req.body;
      const audio = audioData || audioBase64;

      if (!message && !audio) {
        return res.status(400).json({ error: 'Il campo messaggio o audio è obbligatorio.' });
      }

      const isDialogue = mode === 'dialogue' || !mode;
...
- Rispondi SEMPRE nella lingua in cui l'utente ti scrive o parla (${lang === 'ar' ? 'Arabo preferito' : 'Italiano o Arabo'}).
...
- Nelle risposte vocali, mantieni il testo fluido, chiaro e piacevole da ascoltare a voce senza simboli o parentesi ridondanti.`;

      const contents: Array<{ role: string; parts: Array<any> }> = [];
      const historyList = Array.isArray(messages) ? messages : (Array.isArray(history) ? history : []);
      for (const item of historyList.slice(-10)) {
        if (item) {
      ...
      try {
        ...
        const fallbackReply = getOfflineChatReply(message || 'Salam', isDialogue);
        return res.json({ text: fallbackReply, reply: fallbackReply });
      }
    } catch {
      const fallbackReply = getOfflineChatReply(req.body?.prompt || req.body?.message || '', true);
      return res.json({
        text: fallbackReply,
        reply: fallbackReply
      });
  ...
  // 1b. API: Assistente Vocale Multimodale con Audio Diretto
  app.post('/api/ai/audio-chat', async (req, res) => {
    try {
      const { audioBase64, mimeType, history, mode } = req.body;
      if (!audioBase64) {
...
CAPACITÀ APERTA E SENZA LIMITI:
- L'utente ti ha parlato inviandoti un messaggio vocale. Ascolta ciò che dice nel file audio.
- Rispondi con intelligenza, precisione, rispetto ed equilibrio: questioni religiose islamiche, Corano, Hadith, Sunnah, Fiqh, preghiera, digiuno, dua, etica, ma anche riflessioni personali, dialogo aperto, cultura, scienza, consigli pratici di vita quotidiana e curiosità.
- Rispondi SEMPRE nella stessa lingua in cui l'utente ha parlato (se ha parlato in arabo rispondi in arabo, se in italiano rispondi in italiano).
- Rispondi in forma testuale diretta, pronta per essere letta a voce alta (Speech Synthesis): chiara, naturale, senza formule pesanti o parentesi.
${isDialogue ? '- Mantieni uno stile dialogante, piacevole e interattivo invitando l\'utente a continuare la conversazione.' : ''}`;
      ...
      contents.push({
        role: 'user',
        parts: [
          { text: 'Ascolta questa registrazione vocale dell\'utente e rispondi direttamente e calorosamente alla sua voce.' },
          {
            inlineData: {
              mimeType: mimeType || 'audio/webm',
              data: audioBase64
      ...
      try {
        ...
        return res.json({
          reply: 'Ti ascolto con grande piacere! Puoi chiedermi qualsiasi cosa sul Corano, la preghiera a Bastia Umbra o la vita quotidiana.'
        });
      }
    } catch {
      return res.json({
